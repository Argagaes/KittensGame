...
SK.Methods = class {	
	autoTick(state) {
		if (state) {
			sk.autoTickHandle = setInterval(function() {
				game.tick();
			}, 5);
		} else {
			clearInterval(sk.autoTickHandle);
		}
	}
	
	// All credit to https://github.com/lyuhau
	// https://github.com/lyuhau/tm-scripts/blob/master/kittens-game/progress-bar-%26-pre-craft.user.js
	loadPreCraft() {
		if (!sk.preCraft) {
			if (sk.preCraft) {
				return;
			...
			game.msg('PreCraft loaded!');
		} else {
			game.msg('PreCraft already initialized!');
		}
	}
}
